...
from keras.layers import (Activation, Add, Concatenate, Conv1D, Conv2D, Dense,
                          ...
                          Reshape, multiply,BatchNormalization)
import math
from keras.layers.advanced_activations import LeakyReLU
from nets.CSPdarknet53_tiny import DarknetConv2D
def ms_cam(input_feature,r=4,name=""):

	channel = input_feature._keras_shape[-1]
	width = input_feature._keras_shape[-2]
	left_feature = GlobalAveragePooling2D()(input_feature)
	left_feature = Reshape((1, 1, channel))(left_feature)
	print('left_feature.shape', left_feature.shape)
	left_feature = Conv2D(channel//r,kernel_size=1,strides=1)(left_feature)
	left_feature = BatchNormalization()(left_feature)
	left_feature = Activation('relu')(left_feature)
	left_feature = Conv2D(channel, kernel_size=1, strides=1)(left_feature)
	left_feature = BatchNormalization()(left_feature)

	right_feature = Conv2D(channel//r,kernel_size=1,strides=1)(input_feature)
	left_feature = Activation('relu')(left_feature)
	right_feature = Conv2D(channel,kernel_size=1,strides=1)(input_feature)
	right_feature = Add()([right_feature,left_feature])
	right_feature = Activation('sigmoid')(right_feature)
	right_feature = multiply([input_feature, right_feature])
